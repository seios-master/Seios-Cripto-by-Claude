...
t("nenhuma chamada do cliente aponta para api.gdeltproject.org", ()=>{
  ...
  if(diretas.length) throw new Error(diretas.length + " chamada(s) direta(s) restante(s)");
});
...
t("o prazo da rota própria é maior que o do servidor esperando o GDELT", ()=>{
  ...
  if(Number(m[1]) <= 45000) throw new Error("prazo do cliente (" + m[1] + "ms) menor que o do servidor");
});
...
t("429 entra na lista de status que não geram nova tentativa", ()=>{
  ...
  if(!/429/.test(m[1])) throw new Error("o cliente ainda insiste no 429");
});
