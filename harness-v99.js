/* =====================================================================
   ...
   =====================================================================
   Este teste falha contra a v98.1 e contra tudo que veio antes dela — mas
   SÓ SE RODADO ENTRE 21h e 00h de Brasília, que é exatamente o problema
   com bugs de fuso: eles têm horário de expediente. Por isso ele não
   pergunta "que dia é hoje": ele congela o relógio em três instantes e
   confere a resposta em cada um.

   Uso:  node harness-v99.js index.html
   ===================================================================== */
const fs = require("fs");
...
/* congela o relógio: instante real, e o que cada régua diria */
function em(iso, fn){
  const real = Date.now;
...
t("21:07 em Brasília (00:07 UTC do dia seguinte) NÃO pede uma data futura", ()=>{
  // foi este instante que apagou os doze vintages na v98.1
  const d = em("2026-08-16T00:07:00Z", ()=> API.dataFredHoje());
  eq(d, "2026-08-15", "data enviada ao FRED:");
  // e a régua antiga, para comparação
...
t("nunca fica À FRENTE do relógio do FRED, em nenhuma hora do dia", ()=>{
  for(let h = 0; h < 24; h++){
    ...
    // horário central dos EUA: UTC−5 (verão) — o limite superior aceitável
...
t("nenhum outro ponto do código monta realtime_end por conta própria", ()=>{
  ...
  /* a linha do `fredUrl` que COPIA o parâmetro recebido não é uma segunda
     fórmula — é o encanamento. O que não pode existir é outro lugar
     CALCULANDO a data. Por isso a busca é por atribuição com valor, não por
     menção ao nome. */
